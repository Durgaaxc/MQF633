...
int main() {
    ...
    valueDate.day = now_->tm_mday; // Fixed from year to day
    ...
    // Process curveData and update market interest rate curves
    mkt.updateInterestRateCurves(curveData);

    // Update volatility curves
    string volData;
    readFromFile("vol.txt", volData);
    // Process volData and update market volatility curves
    mkt.updateVolatilityCurves(volData);

    // Update bond prices
    string bondPriceData;
    readFromFile("bondprice.txt", bondPriceData);
    // Process bondPriceData and update market bond prices
    mkt.updateBondPrices(bondPriceData);

    // Update stock prices
    string stockPriceData;
    readFromFile("stockPrice.txt", stockPriceData);
    // Process stockPriceData and update market stock prices
    mkt.updateStockPrices(stockPriceData);

    // Create Market and EuropeanOption objects
    Market mkt(Date(2024, 5, 29));
    EuropeanOption europeanOption(Date(2025, 5, 29));

    // Call BlackScholesPrice with pointers
