...
double Pricer::Price(const Market& mkt, Trade* trade) {
  ...
  if (trade->getType() == "TreeProduct") {
    ...
    double price = 0.0; //get from market data
...
double BinomialTreePricer::PriceTree( const Market& mkt, const TreeProduct& trade) {
  // model setup
  double T = (trade.GetExpiry() - mkt.asOf)/365;
  double dt = T / nTimeSteps;
  double stockPrice, vol, rate;
  stockPrice=mkt.getstockprices("AAPL");
  VolCurve v;
  v = mkt.getVolCurve("Volatility");
  RateCurve r;
  r = mkt.getCurve("USD-SOFR");
  vol = v.getVol(mkt.asOf);
  rate = r.getRate(mkt.asOf);
